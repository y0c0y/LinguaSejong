...
#Chat
# Dictionary to store rooms and their messages
rooms = {}
...
def chathome():
    ...
    if request.method == "POST":
        ...
        room = code
        if create != False:
            # Generating unique room code if user wants to create a new room
            room = generate_unique_code(4)
            rooms[room] = {"members": 0, "messages": []}
        elif code not in rooms:
            # Redirecting to home page if room code is invalid
...
def room():
    ...
    if room is None or session.get("name") is None or room not in rooms:
        # Redirecting to home page if session data is invalid
...
def message(data):
    ...
    # Adding message to room messages
    rooms[room]["messages"].append(content)

    # Printing message to console
    print(f"{session.get('name')} said: {data['data']}")

# SocketIO event for connecting to room
@socketio.on("connect")
def connect(auth):
    # Getting session data
    room = session.get("room")
    ...
    # Joining room and sending join message
    join_room(room)
    send({"name": name, "message": "has entered the room"}, to = room)
    rooms[room]["members"] += 1

    # Printing join message to console
    print(f"{name} joined room {room}")

# SocketIO event for disconnecting from room
...
def disconnect():
    ...
    # Removing room if no members are left
    if room in rooms:
        rooms[room]["members"] -= 1
        if rooms[room]["members"] <= 0:
            del rooms[room]
    
    # Sending leave message and printing to console
